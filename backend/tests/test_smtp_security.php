...
function executeMockProbe(string $mode, string $mockScript, ?string $certPath = null): array {
    ...
    $cmd = escapeshellarg($phpBin) . " " . escapeshellarg($mockScript) . " {$port} {$mode} " . escapeshellarg($logFile);
...
$cnf = getenv('OPENSSL_CONF') ?: (file_exists(dirname(PHP_BINARY) . '/extras/ssl/openssl.cnf') ? (dirname(PHP_BINARY) . '/extras/ssl/openssl.cnf') : null);
