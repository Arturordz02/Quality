...
$socket = @fsockopen('127.0.0.1', 8092, $errCode, $errStr, 1);
if (!$socket) {
    echo "Iniciando servidor PHP de prueba en puerto 8092...\n";
    $phpBin = PHP_BINARY ?: 'php';
    $rootDir = escapeshellarg(dirname(__DIR__, 2));
    $cmd = "start /B cmd /C \"set APP_ENV=development& set APP_DEBUG=true& {$phpBin} -S 127.0.0.1:8092 -t {$rootDir}\"";
    pclose(popen($cmd, 'r'));
    sleep(2);
} else {
    fclose($socket);
