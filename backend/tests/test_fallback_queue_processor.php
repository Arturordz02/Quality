...
function skipTest(string $desc, string $reason = 'Requiere base de datos MySQL activa'): void {
...
$phpBin = PHP_BINARY ?: 'php';
$cronCmd = escapeshellarg($phpBin) . ' ' . escapeshellarg(__DIR__ . '/../cron/process-fallback-queue.php');
...
if ($pdo) {
    ...
    skipTest("Cron procesó y eliminó el archivo de contacto de la cola", "Base de datos MySQL inactiva");
    @unlink($contactRes['file_path']);
}
...
if ($pdo) {
    ...
    skipTest("Cron detecta duplicado por queue_id y retira el archivo de disco", "Base de datos MySQL inactiva");
    @unlink($resSameQueueId['file_path']);
}
...
if ($pdo) {
    ...
    @unlink($resContactA['file_path']);
    @unlink($resContactB['file_path']);
}
...
if ($pdo) {
    ...
    skipTest("Cron procesó y eliminó el archivo de reclamación de la cola", "Base de datos MySQL inactiva");
    @unlink($claimRes['file_path']);
}
...
if ($pdo) {
    ...
    skipTest("Cron recuperó y procesó el archivo huérfano .processing", "Base de datos MySQL inactiva");
    @unlink($orphanProcPath);
    @unlink($orphanJsonPath);
}
...
if ($testsPassed + $testsSkipped === $testsTotal) {
